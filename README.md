...
在文章正文点击任意英文单词即可打开语境词典。查询按以下顺序回退，命中即停：

1. **已保存词条**：你自己的生词本；
2. **词典缓存**：此前查过的结果；
3. **本地离线词典**：导入的词库（见下方「导入本地词典」），完全离线可用；
4. **大模型**：已配置文本模型时，结合当前句补充词形、音标、词性、中英文释义和语境说明；
5. **免费翻译 API**：以上都没有结果时的兜底，仅发送该单词本身（不含文章内容）。

查词弹窗会显示释义「来源」；都查不到时会提示如何导入词典或配置模型。第 5 步可在「设置 → 查词与翻译」关闭（关闭后不调用任何外部翻译服务）。

### 导入本地词典（离线查词）

内置词库只含少量高频词。要让绝大多数单词无需联网即可查到释义，导入一份标准词表即可。推荐免费的 [ECDICT](https://github.com/skywind3000/ECDICT)（约 77 万词，下载其中的 `ecdict.csv`）：

```bash
python scripts/import_dictionary.py ecdict.csv
```

脚本会自动识别 ECDICT 表头（`word, phonetic, definition, translation, ...`）以及通用的 `word,translation` 两列 CSV（后者可加 `--no-header`）。重复导入会原地更新。导入后可在「设置 → 查词与翻译」看到已收录词条数。
...
- 查看本地词典收录量，并开关「无本地释义时的免费翻译 API 兜底」；
- 调整阅读主题、字号与行距。
