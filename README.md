...
| 原版音频 | 单行图标播放器：上一句/播放/下一句、进度条、倍速；桌面直接进入听写与跟读，手机从“⋯”菜单进入；播放位置与语速保存在服务器，可跨设备继续 |
| 跟读高亮 | 优先读取缓存的原版音频时间轴，必要时自动准备；播放和拖动进度时高亮当前句，并在句子离开画面时跟随滚动 |
| AI 句子精读 | 点击完整句子，依次查看参考译文、重点生词、句子主干、长难句拆分；关闭与上一句/下一句按钮始终可见；成功结果缓存在服务器 |
| 全文预分析 | 一键按阅读顺序排队分析；跳过有效缓存，支持暂停、继续和失败重试；离开页面仍继续，刷新后读取进度 |
| 原音跟读 | 选句播放，支持 1/2/3 遍、0/2/4/6 秒跟读停顿；最后一遍结束自动暂停 |
| 原音听写 | 听写弹窗先隐藏原句，提交后逐词标出漏词、错词、多写，并保存成绩与练习记录 |
...
| 外部播放器 | 通过只读 HTTPS WebDAV 在 foobar2000 等手机播放器中收听；自动按杂志、期号、栏目分类，每台设备使用可单独吊销的密码 |
...
旧的整篇教学分析接口（文本检查、总览、段落分析、阅读题和学习包等）及多面板前端已移除。现在的“一键分析全文”只预生成阅读页逐句解析；原音听写复用现有逐词评分和学习记录接口，不恢复旧的复杂页面。
...
- 一键分析会调用已配置的 AI 服务并产生相应费用。按顺序逐句生成，点击已完成的句子直接读取缓存；未完成的句子仍需等待。重复句子只分析一次。暂停会等待当前请求结束；失败的句子可重试。
- 分析队列进度保存在数据库中。浏览器关闭不影响后台任务；服务器重启后显示为暂停，点击继续即可用缓存接着处理。使用默认单进程服务运行队列，同时最多处理两篇文章。
- 新版解析包含重点生词；旧版缓存保留，新格式首次使用时重新生成。已收藏句子和学习记录不受影响。
- 上一句/下一句、跟读和听写依赖原音时间轴；准备期间按钮不可用。补间句的边界为估算。正常“播放”连续播放整篇；选句播放才使用跟读次数和停顿设置。
- 播放器桌面高 60px、手机高 56px（另加设备底部安全区）。手机拖动进度条时显示时间，平时隐藏；重新对齐位于“⋯”菜单。跟读设置向上浮出，点击外部或按 Esc 收起，不挤占正文高度。
...
此功能仅在 HTTPS 部署中开放。在“设置 → 在 foobar2000 等外部播放器中收听”打开设置页，为手机生成一个独立设备密码，再按页面指引添加 WebDAV 服务器。密码只显示一次；手机丢失或不再使用时可单独吊销，不影响网站登录密码。

已与文章配对的音频显示为“杂志 → 期号 → 栏目 → 顺序编号 + 文章标题”；尚未配对的音频保留在“未配套”中。WebDAV 全程只读，外部播放器无法删除或修改服务器音频。后台播放、循环和倍速由 foobar2000 等播放器控制。
