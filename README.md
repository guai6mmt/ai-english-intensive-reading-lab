...
| 按包管理 | 查看每次音频导入记录，将该包中新建的音频整批移入回收站或恢复；重复引用不会误删历史音频 |
...
导入后可在“音频库 → 导入包”查看每次导入的使用中、回收站、重复文件数量，并按包整批移入回收站或恢复。整包操作只影响该次任务实际新建的音频，不会删除因 SHA-256 去重而指向的历史文件。删除后重新导入同一包时，回收站音频会自动恢复并转移到新的导入记录，不会变成不可见的“重复文件”。
...
2. 点击“自动匹配”。系统会识别数字日期和英文月份日期，并综合栏目、顺序、标题相似度与篇幅/时长；音频使用短标题或比 EPUB 少收录几篇时，也会保持顺序对齐并提示复核。
...
通过“导入一期 EPUB + 音频 ZIP”同步导入时，系统会在快速匹配后自动核验低置信度、未匹配或使用编辑部短标题的项目；高置信度项目不会调用 ASR。在配套管理中也可以手动点击“正文核验”。系统从每段长音频抽取开头和两个正文中段（通常共约 84 秒），调用 Qwen ASR 后，用词组重合度与同栏目的文章正文进行一对一比较。识别文本按音频哈希和模型版本缓存在服务器；再次核验不会重复调用 ASR。此功能需要在设置中配置 Qwen/DashScope API Key 与 OSS，并在服务器安装 `ffmpeg`；未配置或核验失败时，导入仍会完成，普通的期号、栏目、顺序及人工匹配仍可使用。
