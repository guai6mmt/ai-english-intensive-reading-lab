...
一个围绕 **读文章 → 听原音 → 点句翻译 → 收藏复习** 的私人英语学习系统。

## 功能

| 功能 | 使用方式 |
| --- | --- |
| 文章库 | 导入 EPUB、DOCX、TXT；按刊物分组，搜索标题与栏目，点击文章开始阅读 |
| 舒适阅读 | 电脑采用居中单栏，手机自动适配；保留原文段落；标准、大、特大三档字号 |
| 原版音频 | 播放与文章关联的原音频；底部播放器支持暂停、拖动进度、倍速；播放位置与语速保存在服务器，可跨设备继续 |
| AI 句子翻译 | 点击完整句子，结合文章上下文翻译成中文；支持 DeepSeek、通义千问及其兼容服务地址；成功译文缓存在服务器 |
...
- 本版支持边听边读，不会使用估算时间假装逐句同步高亮。
- 首次翻译需要网络和有效 AI 配置；调用失败会明确提示，不使用本地占位解释冒充 AI 译文。再次点击可重试。
- 句子翻译结果、收藏、复习和音频进度存放在服务器；字号及上次点击的句子位置存放在当前浏览器。
