...
对于“建议复核”或音频使用编辑部短标题的项目，可以点击“正文核验”。系统只处理低置信度候选，从每段长音频抽取开头和两个正文中段（通常共约 84 秒），调用 Qwen ASR 后，用词组重合度与同栏目的文章正文进行一对一比较。识别文本按音频哈希和模型版本缓存在服务器；再次核验不会重复调用 ASR。此功能需要在设置中配置 Qwen/DashScope API Key 与 OSS，并在服务器安装 `ffmpeg`；未配置时普通的期号、栏目和顺序匹配仍可使用。
